...
class Dastan:
    ...
    def __CheckSquareIsValid(self, SquareReference, StartSquare):
        if not self.__CheckSquareInBounds(SquareReference):
            return False
        if self._Board[self.__GetIndexOfSquare(SquareReference)].ContainsBarrier():return False
    ...
    def PlayGame(self):
        GameOver = False
        self.__PlaceBarrier('Player 1')
        self._CurrentPlayer=self._Players[1]
        self.__PlaceBarrier('Player 2')
        self._CurrentPlayer=self._Players[0]
        while not GameOver:
            self.__DisplayState()
            SquareIsValid = False
            ...
            if MoveLegal and self.__CalculateManhattanDistance(StartSquareReference, FinishSquareReference):
    ...
    def __CalculateManhattanDistance(self, StartSquareReference, FinishSquareReference):
        deltax,deltay=abs((StartSquareReference-FinishSquareReference)%10),abs((StartSquareReference-FinishSquareReference)//10)
        for y in range(1,deltay+1):
            Flag=False
            for x in range(1,deltax+1):
                if not self._Board[self.__GetIndexOfSquare()].ConstainsBarrier():
                    Flag=True
            if not Flag:
                return False
        else:
    ...
    def __PlaceBarrier(self,Name):
        flag=False
        while not flag:
            StartSquareReference = int(input(Name + ', choose a position to place your barrier: '))
            modifier = 'B' if self._CurrentPlayer.SameAs(self._Players[0]) else 'b'
            if self.__CheckSquareIsValid(StartSquareReference, False) and self.__CheckSquareIsValid(StartSquareReference-1, False) and self.__CheckSquareIsValid(StartSquareReference+1, False):
                self._Board[self.__GetIndexOfSquare(StartSquareReference-1)]=Barrier(self._CurrentPlayer,modifier)
                self._Board[self.__GetIndexOfSquare(StartSquareReference)]=Barrier(self._CurrentPlayer,modifier)
                self._Board[self.__GetIndexOfSquare(StartSquareReference+1)]=Barrier(self._CurrentPlayer,modifier)
                flag=True
            else:
