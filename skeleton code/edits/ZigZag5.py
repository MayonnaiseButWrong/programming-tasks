    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3) or 9 to take the offer or 8 to spy on "+self._OtherPlayer.GetName()+"'s move option queue: "))
                if Choice == 9:
                    self.__UseMoveOptionOffer()
                    self.__DisplayState()
                elif Choice == 8:
                    print('')
                    print(self._OtherPlayer.GetJustQueue())
...
class Player:
    ...
    def GetJustQueue(self):
        return self.__Name + "'s move option queue: " + self.__Queue.GetQueueAsString() + "\n"
