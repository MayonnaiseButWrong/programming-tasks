...
class Dastan:
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            if MoveLegal:
                if self._CurrentPlayer.GetFreeTurn()==False:
    ...
    def __CreateBoard(self):
        for Row in range(1, self._NoOfRows + 1):
            for Column in range(1, self._NoOfColumns + 1):
                ...
                elif Row == (self._NoOfRows//2 + 1) and Column ==self._NoOfColumns // 2 + 1:
                    S = Taziz()
...
class Taziz(Square):
    ...
    def RemovePiece(self):
        super(Taziz, self).RemovePiece()
        self._CampedTurns = 0
        self._Symbol = 'x'
    
    def CheckCamp(self):
        if not self._BelongsTo==None:
            self._CampedTurns+=1
        if self._CampedTurns>=2:
            self._BelongsTo.SetFreeTurn(True)
...
class Player:
    ...
    def GetFreeTurn(self):
        return self.__FreeTurn
    
    def SetFreeTurn(self,v):
        self.__FreeTurn=v
