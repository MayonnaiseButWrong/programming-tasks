...
class Dastan:
    ...
    def __AwardWafr(self):
        value=random.choice([False,False,False,True])
        #value=True
        if value is True:
            self._CurrentPlayer.SetWafr(True)
        else:
            self._CurrentPlayer.SetWafr(False)
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            SquareIsValid = False
            Choice = 0
            WafrValue=self._CurrentPlayer.GetWafr()
            if WafrValue is True:
                while Choice < 1 or Choice > 5:
                    print('you have been awarded the Wafr, you may choose an move option from the queue for free this turn')
