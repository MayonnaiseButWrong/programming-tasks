...
#start edit
    def __CalculateSahmMove(self, StartSquareReference):
        d=self._CurrentPlayer.GetDirection()
        y=StartSquareReference//10 + d
        x=StartSquareReference%10
        points=0
        while y>0 and y<self._NoOfRows:
            if not self._Board[self.__GetIndexOfSquare(y*10+x)].ContainsKotla():
                points+=self.__CalculatePieceCapturePoints(y*10+x)
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                ...
                if Choice == 9:
                    ...
                    self.__DisplayState()
#Start edit
                elif self._CurrentPlayer.ChoiceIsSahm(Choice-1):
                    if self._CurrentPlayer.GetSahmUsed():
                        print('you have aready used the Sahm thus game, you cant use it again in this game')
                        print('please pick a different piece')
                        Choice = 0
            ...
            if self._CurrentPlayer.ChoiceIsSahm(Choice-1) and not self._CurrentPlayer.GetSahmUsed():
                self._CurrentPlayer.SetSahmUsed(True)
                self._CurrentPlayer.ChangeScore(-(Choice + (2 * (Choice - 1))))
                self.__UpdatePlayerScore(self.__CalculateSahmMove(StartSquareReference))
            else:
#end edit
                SquareIsValid = False
                while not SquareIsValid:
...
#start edit
    ...
    def ChoiceIsSahm(self,pos):
        if self.__Queue.GetMoveOptionInPosition(pos).GetName()=='sahm':
            return True
        else:
            return False
#end edit
