...
class Dastan:
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            SquareIsValid = False
            Choice = 0
            while (Choice < 1 or Choice > 3) and not Choice == 7:
                Choice = int(input("Choose move option to use from queue (1 to 3) or 9 to take the offer or 7 to replace the curret piece wit a kotla: "))
                if Choice == 9:
            ...
            if Choice==7:
                self.__ReplaceWithKotla(StartSquareReference)
            else:
                SquareIsValid = False
                while not SquareIsValid:
    ...
    def __ReplaceWithKotla(self, square):
