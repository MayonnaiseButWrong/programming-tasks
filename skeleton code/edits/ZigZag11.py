    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3) or 9 to take the offer or 6 to open the sub options menu: "))
                if Choice == 9:
                    self.__UseMoveOptionOffer()
                    self.__DisplayState()
                elif Choice == 6:
                    choice2 = 'q'
                    while not choice2.lower() in ['a','b','c','d']:
...
''')
                        choice2 = str(input('choose a, b, c, d, or e: '))
                        if choice2.lower()=='e':
                            break
                    else:
                        if choice2 == 'a':
                            ...
                            self._CurrentPlayer.ReplaceQueue(self._OtherPlayer.ReplaceQueue(self._CurrentPlayer.GetQueue()))
                        elif choice2 == 'c':
                            self._CurrentPlayer.SwapFirstAndLast()
                        elif choice2 == 'd':
                            pos = int(input('What is the position of the element you want to make to the front of your Move Option Queue?: '))
...
class MoveOptionQueue:
    ...
    def SwapFirstAndLast(self):
        temp = self.__Queue[0]
        self.__Queue[0] = self.__Queue[-1]
        self.__Queue[-1] = temp
    
    def MoveItemToFront(self,pos):
        self.__Queue.append(self.__Queue.pop(pos))
...
class Player:
    ...
    def ReverseQueue(self):
        ...
        print("Move option queue: " + self.__Queue.GetQueueAsString())
    ...
    def  MoveItemToFront(self,pos):
        self.__Queue.MoveItemToFront(pos)
        print("Move option queue: " + self.__Queue.GetQueueAsString())
        
    def GetQueue(self):
        return self.__Queue
    
    def ReplaceQueue(self,q):
        temp=copy.deepcopy(self.__Queue)
        self.__Queue=q
        print(self.__Name+"'s Move option queue: " + self.__Queue.GetQueueAsString())
        return temp
