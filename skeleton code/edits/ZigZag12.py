...
class Dastan:
    ...
    def CountNormalPieces(self):
        count=0
        for S in self._Board:
            PieceInSquare = S.GetPieceInSquare()
            if self._CurrentPlayer.SameAs(PieceInSquare.GetBelongsTo()) and not PieceInSquare.GetTypeOfPiece() == "mirza":
                count+=1
        return count
    
    def _CheckReincarnation(self,FinishSquareReference):
        y=FinishSquareReference//10
        if (y == 50 and self._CurrentPlayer.SameAs(self._Players[0])) or (y == 10 and self._CurrentPlayer.SameAs(self._Players[0])):
            if self.CountNormalPieces()<self._NoPieces:
                return True
    
    def _Reincarnate(self,FinishSquareReference):
        y=FinishSquareReference//10
        x = random.randint(0,5)
        while not self._Board[self.__GetIndexOfSquare(y + x)].GetPieceInSquare()==None:
            x = random.randint(0,5)
        self._Board[self.__GetIndexOfSquare(y + x)].RemovePiece()
        if self._CurrentPlayer.SameAs(self._Players[0]):
            CurrentPiece = Piece("piece", self._CurrentPlayer, 1, "!")
        else:
            CurrentPiece = Piece("piece", self._CurrentPlayer, 1, '"')
        self._Board[self.__GetIndexOfSquare(y + x)].SetPiece(CurrentPiece)
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            Reincarnation = self._CheckReincarnation(FinishSquareReference)
            if Reincarnation:
                self._Reincarnate(FinishSquareReference)
            if MoveLegal:
